...
// The memberships endpoint caps `top` well below the 1000 the customers
// endpoint allows, so page in smaller chunks.
const MEMBERSHIP_PAGE = 100;

export async function listMembershipsByProgram({ status } = {}) {
  const all = [];
  for (let skip = 0; ; skip += MEMBERSHIP_PAGE) {
    const url =
      `${hosts().memberships}/api/memberships` +
      `?programId=${encodeURIComponent(config.programId)}&top=${MEMBERSHIP_PAGE}&skip=${skip}`;
    const page = await get(url);
    const docs = page?.docs ?? [];
    all.push(...docs);
    if (!docs.length || all.length >= (page?.total ?? 0)) break;
  }
...
export async function listAccessListEntries() {
  const res = await get(`${hosts().core}/api/access-lists/${config.accessListId}/entries?top=1000`);
  return res?.docs ?? (Array.isArray(res) ? res : []);
}
